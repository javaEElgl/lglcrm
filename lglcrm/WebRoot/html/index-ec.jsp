...
<script src="/lglcrm/js/jquery-3.2.1.min.js"></script>
...
function initialise() {
	...
	$.post("findroleright",function(data){
		var arr=new Array();
		arr=data.split("-");
       l2ID = theMenu.addEntry(-1, "Folder", "客户关系管理系统", "/lglcrm/html/mywork.jsp", "客户关系管理系统");
       if(arr[0]=="true"||arr[1]=="true"){
		L01 = theMenu.addChild(l2ID, "Folder", "营销管理", "", "营销管理...");
		}
	
	if(arr[0]=="true"){
	L0101 = theMenu.addChild(L01, "Document", "销售机会管理", "sale_find", "销售机会管理...");
	}
	if(arr[1]=="true"){
	L0102 = theMenu.addChild(L01, "Document", "客户开发计划", "plan_find", "客户开发计划...");
	}
	...
	if(arr[2]=="true"){
	L0201 = theMenu.addChild(L02, "Document", "客户信息管理", "customer_find", "客户信息管理...");
	}
	if(arr[3]=="true"){
	L0201 = theMenu.addChild(L02, "Document", "客户流失管理", "lost_find", "客户流失管理...");
	}	
	
	
	if(arr[4]=="true"||arr[5]=="true"||arr[6]=="true"||arr[7]=="true"||arr[8]=="true"){
	L03 = theMenu.addChild(l2ID, "Folder", "服务管理", "", "客户服务管理...");
	}
	if(arr[4]=="true"){
	L0301 = theMenu.addChild(L03, "Document", "服务创建", "servicer_find?flag=1", "服务创建...");
	}
	if(arr[5]=="true"){
	L0302 = theMenu.addChild(L03, "Document", "服务分配", "servicer_find?flag=2", "服务分配...");
	}
	if(arr[6]=="true"){
	L0303 = theMenu.addChild(L03, "Document", "服务处理", "servicer_find?flag=3", "服务处理...");
	}
	if(arr[7]=="true"){
	L0304 = theMenu.addChild(L03, "Document", "服务反馈", "servicer_find?flag=4", "服务反馈...");
	}
	if(arr[8]=="true"){
	L0305 = theMenu.addChild(L03, "Document", "服务归档", "servicer_find?flag=5", "服务归档...");
	}
	
	if(arr[9]=="true"||arr[10]=="true"||arr[11]=="true"||arr[12]=="true"){
	L04 = theMenu.addChild(l2ID, "Folder", "统计报表", "", "统计报表...");
	}
	if(arr[9]=="true"){
	L0401 = theMenu.addChild(L04, "Document", "客户贡献分析", "orders_rept", "客户贡献分析...");
	}
	if(arr[10]=="true"){
	L0402 = theMenu.addChild(L04, "Document", "客户构成分析", "customer_rept", "客户构成分析...");
	}
	if(arr[11]=="true"){
	L0403 = theMenu.addChild(L04, "Document", "客户服务分析", "servicer_rept?flag=1", "客户服务分析...");
	}
	if(arr[12]=="true"){
	L0404 = theMenu.addChild(L04, "Document", "客户流失分析", "lost_rept", "客户流失分析...");
	}
	if(arr[13]=="true"||arr[14]=="true"||arr[15]=="true"){
	L05 = theMenu.addChild(l2ID, "Folder", "基础数据", "", "基础数据...");
	}
	if(arr[13]=="true"){
	L0501 = theMenu.addChild(L05, "Document", "数据字典管理", "dict_find", "数据字典管理...");
	}
	if(arr[14]=="true"){
	L0502 = theMenu.addChild(L05, "Document", "查询产品信息", "product_find?flag=product", "查询产品信息...");
	}
	if(arr[15]=="true"){
	L0503 = theMenu.addChild(L05, "Document", "查询库存", "product_find?flag=storage", "查询库存...");
	}
	if(arr[16]=="true"||arr[17]=="true"){
        L06 = theMenu.addChild(l2ID, "Folder", "权限管理", "", "权限管理...");
       }
    if(arr[16]=="true"){
	L0601 = theMenu.addChild(L06, "Document", "用户管理", "user_find", "用户管理...");
	}
	if(arr[17]=="true"){
	L0602 = theMenu.addChild(L06, "Document", "角色授权管理", "role_find", "角色授权管理...");
	}
...
if (theBrowser.IE){
	self.document.writeln('<frameset rows="*" cols="0%,*" border="1"  noresize framespacing="2">'); 
	...
	self.document.writeln('<frameset  cols="15%,*" border="3"  noresize framespacing="2">'); 
	self.document.writeln('<frame name="menu" src="menu.jsp"  border="3" scrolling="auto" marginwidth="0"  leftmargin="0" marginheight="0" APPLICATION="yes">');
	self.document.writeln('<frame name="text" src="mywork.jsp"  frameborder="no"  marginwidth="0" marginheight="0" APPLICATION="yes">');
	self.document.writeln('</frameset>');
}

...
<!--
	...
	function getDHTMLObj(objName) {
		return eval('document' + theBrowser.DHTMLRange + '.' + objName
				+ theBrowser.DHTMLStyleObj);
	}
	function getDHTMLObjHeight(objName) {
		return eval('document' + theBrowser.DHTMLRange + '.' + objName
				+ theBrowser.DHTMLDivHeight);
	}
	...
	function drawStatusMsg() {
		if (document.layers) {
			...
			if (document.all) {
				document.styleSheets["JoustStyles"].addRule("#statusMsgDiv",
						"top:" + menuStart);
			}
		}
		document
				.writeln('<DIV ID="statusMsgDiv"><CENTER>请稍等...</CENTER></DIV>');
	}
	function drawLimitMarker() {
		var b = theBrowser;
		if (theMenu && b.hasDHTML && b.needLM) {
			var limitPos = theMenu.maxHeight + menuStart
					+ getDHTMLObjHeight('menuBottom');
			if (b.code == 'NS') {
...
<BODY LINK="#FFFFFF" marginwidth="1" marginheight="1"
	...
	bgcolor="#0066cc" text="#FFFFFF" vlink="#FFFFFF" alink="#FFFFFF">
	<DIV ID="menuTop">&nbsp;</DIV>
	<SCRIPT LANGUAGE="JavaScript">
	<!--
		setTop();
	...
	<!--这些字体，�色等参�应用于后面的菜� -->
	<FONT FACE="宋体" color="#FFF"> <SCRIPT LANGUAGE="JavaScript">
	<!--
		if (theMenu) {
